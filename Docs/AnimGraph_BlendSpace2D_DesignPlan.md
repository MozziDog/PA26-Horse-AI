...
| 2026-07-14 | Windows(사용자) | **Build 4 UX 폴리시** — (1) 캔버스 내부 패딩으로 축범위 경계 샘플 가시성 개선. (2) 샘플 리스트 스크롤 child + 캔버스↔리스트 **선택 공유**(`GetBSCanvasState`, 선택 시 하이라이트·자동 스크롤). (3) My Blueprint 네비게이터에 **Blend Spaces 섹션** 노출(클릭 시 루트 그래프 선택·포커스) + BlendSpace **이름 편집**(inspector Name 필드 → State SubGraph 드롭다운에서 식별). | MSVC Release x64 빌드 성공(EXITCODE=0). |
