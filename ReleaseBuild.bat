...
set PROJECT_DIR=%SOLUTION_DIR%AppleJamEngine
...
msbuild "%SOLUTION_DIR%AppleJamEngine.sln" /p:Configuration=Release /p:Platform=x64 /m /v:minimal
if %ERRORLEVEL% neq 0 (
    echo BUILD FAILED
    pause
    exit /b 1
...
copy "%BUILD_OUTPUT%\AppleJamEngine.exe" "%RELEASE_BIN%\" >nul
...
echo @echo off
echo cd /d "%%~dp0"
echo start "" "%%~dp0Bin\AppleJamEngine.exe"
